...
function giveFinancialAdvice() {
  ...
  rows.forEach(row => {
    ...
    const amount = parseFloat(cells[2]?.innerText.replace(/[^0-9.-]+/g, "")) || 0;
  ...
  // Clear any previous advice (if any)
  const adviceSection = document.getElementById('ai-advice');
  adviceSection.innerText = "";  // Clear previous advice
  ...
  // 3️ SUCCESS: Income is significantly higher than expenses (fixed condition)
  else if (totalIncome > 0 && totalIncome > totalExpenses && totalIncome >= totalExpenses * 2) {
    advice = "🎉 Great job! Your income is significantly higher than your expenses. Consider saving or investing.\n";
  }
  // 4️ HEALTHY BALANCE: Only if the user has income and expenses
  ...
  // 5️ NO EXPENSES: If the user only has income
...
function saveToDatabase() {
  ...
  const passcode = prompt('Enter a passcode to save your data:');

  // ✅ Save data under the passcode in Firebase
  const userRef = window.ref(window.database, 'users/' + passcode);
  window.set(userRef, {
    startingBalance,
    currentBalance,
    transactions
...
function retrieveFromDatabase() {
  ...
  const passcode = prompt('Enter your passcode to retrieve your data:');
  ...
  window.get(userRef)
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        alert('No data found for this passcode.');
      }
    })
    .catch((error) => {
